...
const NoticesLiner = require('../models/NoticeLiner');
...
router.post('/:schoolCode', (req, res) => {
    const { schoolCode, message } = req.body

    console.log(schoolCode, message)
    const NoticeLiner = new NoticesLiner({ schoolCode, message });
    NoticeLiner.save()
        .then(() => {
            res.status(201).json(NoticeLiner);
        })
        .catch((error) => {
...
router.get('/:schoolCode', (req, res) => {
    ...
    NoticesLiner.find({ schoolCode }) // Assuming schoolCode is a property in your SliderImgs schema
        .then((slider) => {
            if (!slider) {
...
router.delete('/:id', (req, res) => {
    ...
    NoticesLiner.findByIdAndRemove(id)
        .then((NoticeLiner) => {
            if (!NoticeLiner) {
                return res.status(404).json({ error: 'NoticeLiner not found' });
            }
            res.json({ message: 'NoticeLiner deleted successfully' });
        })
        .catch((error) => {
            res.status(500).json({ error: 'An error occurred' });
        });
